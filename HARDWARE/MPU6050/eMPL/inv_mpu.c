    ...
    unsigned char *more)
{
    ...
    if (!st.chip_cfg.dmp_on)
		{
//				printf("I'm in mpu_read_fifo_stream at 1791\n");
        return -1;
    }
		if (!st.chip_cfg.sensors)
		{
//			printf("I'm in mpu_read_fifo_stream at 1796\n");
        return -1;
		}
    if (i2c_read(st.hw->addr, st.reg->fifo_count_h, 2, tmp))
		{
//			printf("I'm in mpu_read_fifo_stream at 1801\n");
        return -1;
		}
    fifo_count = (tmp[0] << 8) | tmp[1];
    if (fifo_count < length) {
//			printf("I'm in mpu_read_fifo_stream at 1806\n");
        more[0] = 0;
        return -1;
    }
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        ...
        if (i2c_read(st.hw->addr, st.reg->int_status, 1, tmp))
				{
//					printf("I'm in mpu_read_fifo_stream at 1814\n");
            return -1;
        }
					if (tmp[0] & BIT_FIFO_OVERFLOW) {
//						printf("I'm in mpu_read_fifo_stream at 1818\n");
            mpu_reset_fifo();
            return -2;
        }
    }
